...
impl Renderer {
    ...
    /// Renders everything through ratatui
    fn render_playground_frame(&mut self, delta_seconds: f64) -> Result<(), RendererError> {
        let _ = debug_log("render_playground_frame called");
        
        // Ensure TUI is healthy
        ...
        // Prepare overlay data if visible
        let overlay_data = if self.overlay_visible {
            Some(self.prepare_overlay_data())
        } else {
            None
        };
        ...
        let toast = self.toast_text.clone();
        
        // Get the TUI terminal
        let term = self.tui.as_mut().ok_or_else(|| {
            RendererError::Other("TUI unexpectedly None".to_string())
        })?;
        
        // Render everything in one ratatui draw call
        term.draw(|f| {
            use ratatui::layout::{Layout, Direction, Constraint, Rect};
            use ratatui::widgets::{Block, Borders, Clear, List, ListItem};
            use ratatui::style::{Color as TuiColor, Modifier, Style};
            use pattern_widget::PatternWidget;
            
            let size = f.size();
            
            // First, render the pattern background covering the entire terminal
            let pattern_widget = PatternWidget::new(&content, &engine, time as f64);
            f.render_widget(pattern_widget, size);
            
            // Then render overlay on top if visible
            if let Some(ref overlay) = overlay_data {
                // Compact floating panel
                let overlay_height = (12u16).min(size.height.saturating_sub(4));
                let panel_w = ((size.width as u32 * 90) / 100) as u16;
                let panel_w = panel_w.max(48).min(size.width.saturating_sub(2));
                let start_y = size.height.saturating_sub(overlay_height + 2);
                let start_x = ((size.width.saturating_sub(panel_w)) / 2).max(1);
                ...
                f.render_widget(overlay_block, overlay_rect);
                ...
                let selected_color = TuiColor::Yellow;
                for (i, title) in titles.iter().enumerate() {
                    let style = if i == overlay.section as usize {
                    ...
                    f.render_widget(title_widget, title_chunks[i]);
                }
                ...
                // Render pattern list
                let visible_rows = inner[1].height as usize;
                let pattern_items: Vec<ListItem> = overlay.pattern_names
                    .iter()
                    .skip(overlay.pattern_offset)
                    .take(visible_rows)
                    .enumerate()
                    .map(|(i, name)| {
                        let is_selected = overlay.pattern_offset + i == overlay.pattern_sel;
                ...
                f.render_widget(pattern_list, content_chunks[0]);
                
                // Render params list
                let param_items: Vec<ListItem> = overlay.param_names
                    .iter()
                    .skip(overlay.param_offset)
                    .take(visible_rows)
                    .enumerate()
                    .map(|(i, name)| {
                        let is_selected = overlay.param_offset + i == overlay.param_sel;
                        let style = if is_selected {
                            Style::default().bg(TuiColor::Blue).fg(TuiColor::White)
                        } else {
                            Style::default()
                ...
                f.render_widget(param_list, content_chunks[1]);
                
                // Render themes list
                let theme_items: Vec<ListItem> = overlay.theme_names
                    .iter()
                    .skip(overlay.theme_offset)
                    .take(visible_rows)
                    .enumerate()
                    .map(|(i, name)| {
                        let is_selected = overlay.theme_offset + i == overlay.theme_sel;
                        let style = if is_selected {
                            Style::default().bg(TuiColor::Blue).fg(TuiColor::White)
                        } else {
                            Style::default()
                ...
                f.render_widget(theme_list, content_chunks[2]);
                
                // Render art list
                let art_items: Vec<ListItem> = overlay.art_names
                    .iter()
                    .skip(overlay.art_offset)
                    .take(visible_rows)
                    .enumerate()
                    .map(|(i, name)| {
                        let is_selected = overlay.art_offset + i == overlay.art_sel;
                        let style = if is_selected {
                            Style::default().bg(TuiColor::Blue).fg(TuiColor::White)
                        } else {
                            Style::default()
                ...
                f.render_widget(art_list, content_chunks[3]);
                
                // Footer with controls
                let footer_text = "[Tab] switch • [↑↓] select • [Enter] apply • [;] toggle overlay";
    ...
    /// Prepare overlay data for rendering
    fn prepare_overlay_data(&self) -> OverlayData {
        OverlayData {
            pattern_names: self.available_patterns.clone(),
            theme_names: self.available_themes.clone(),
            art_names: self.available_arts.clone(),
            param_names: self.current_param_names(),
            pattern_sel: self.overlay_pattern_sel,
            theme_sel: self.overlay_theme_sel,
            art_sel: self.overlay_art_sel,
            param_sel: self.overlay_param_sel,
            pattern_offset: self.overlay_pattern_offset,
            theme_offset: self.overlay_theme_offset,
            art_offset: self.overlay_art_offset,
            param_offset: self.overlay_param_offset,
            section: self.overlay_section,
        }
    }

    // Old overlay method removed - everything now goes through render_playground_frame
    
    fn render_overlay_old(&mut self) -> Result<(), RendererError> {
    ...
    pub fn render_frame(&mut self, text: &str, delta_seconds: f64) -> Result<(), RendererError> {
        // Always use unified ratatui rendering when overlay is visible or in demo mode
        if self.overlay_visible || self.demo_mode {
            // Store the text content if it's different
            if text != self.content {
                self.content = text.to_string();
                self.buffer.prepare_text(&self.content)?;
            }
            return self.render_playground_frame(delta_seconds);
        }
        ...
        if self.overlay_visible {
            ...
            // Overlay is now rendered as part of render_playground_frame
    ...
    pub fn handle_resize(&mut self, new_width: u16, new_height: u16) -> Result<(), RendererError> {
        ...
        self.engine = self.engine.recreate(new_width as usize, new_height as usize);
        let _ = debug_log(&format!("Resized pattern engine to {}x{}", new_width, new_height));
        
        // Update TUI terminal size if it exists
    ...
    fn draw_full_screen(&mut self) -> Result<(), RendererError> {
        // In playground/demo mode, always use ratatui rendering
        if self.demo_mode || self.overlay_visible {
            // Use a small delta for immediate update
            self.render_playground_frame(0.016)?;
        } else {
            // Legacy path for non-demo mode without overlay
            // This should eventually be removed once everything is ratatui
            let mut stdout = self.terminal.stdout();
            let visible_range = self.scroll.get_visible_range();
    ...
    pub fn set_demo_art(&mut self, art: &str) -> Result<(), RendererError> {
        ...
        // Update pattern engine dimensions to match terminal size
        let (width, height) = if let Some(ref tui) = self.tui {
            let size = tui.size().unwrap_or(ratatui::layout::Rect { x: 0, y: 0, width: 80, height: 24 });
        ...
        // Recreate pattern engine with correct dimensions
        self.engine = self.engine.recreate(width, height);
        let _ = debug_log(&format!("Updated pattern engine dimensions to {}x{}", width, height));
